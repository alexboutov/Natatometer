 ...
 * 
 * Bug fix: Timestamp underflow on first sample
 *   - Root cause: currentTime captured at loop start, but recordingStartTime
 *     set later after file operations, causing currentTime < recordingStartTime
 *   - Fix: Set recordingStartTime and lastSampleTime AFTER all blocking 
 *     operations (file open, beep) to ensure first sample has valid timestamp
 */
...
void startRecording() {
  // Open file first (this takes time due to flash operations)
  ...
  // This is critical: ensures first sample will have currentTime >= recordingStartTime
  unsigned long now = millis();
  recordingStartTime = now;
  lastSampleTime = now;  // Sync sample timer to prevent immediate sampling with stale time
...
void stopRecording() {
  // Disable recording first to prevent race conditions
  isRecording = false;
